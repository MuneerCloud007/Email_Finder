...
const createTransport = () => {
  ...
  export const send = async (data) => {
    ...
    if (rex.test(data.to.toLowerCase())) {
      ...
      // Get content from DB
      let content = email.filter(email => email.name === data.template && email.locale === data.locale)[0];
      content = { ...content, id: data.id };
  
      const html = await createEmail({ template: data.html_template || 'template', content, values: data.content });
   ...
   */
  const createEmail = async ({ template, content, values }) => {
    const domain = "noreply@webapp.onlylwc.com";
    const usersPath = path.join(__dirname, '..', '..', 'src', 'emails', `${template}.html`);
    let email;
